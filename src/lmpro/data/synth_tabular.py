...
def split_offset(split: str) -> int:
    """Deterministic, distinct seed offset for a dataset split"""
...
class SyntheticTabularDataset(Dataset):
    ...
    def __init__(
        self, config: TabularDatasetConfig, task: str = "classification", split: str = "train", normalize: bool = True
    ):
        self.config = config
        self.task = task
        self.split = split
    ...
    def _split_slice(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """Return this split's disjoint slice of a pooled dataset.

        sklearn's ``make_*`` helpers draw a *different* generating function for every
        ``random_state``. Seeding per split would therefore give validation and test
        sets from a different problem than training. Instead one pool is generated from
        the base seed and each split takes its own contiguous, non-overlapping block.
        """
        n = self.config.num_samples
        start = split_offset(self.split) * n
        return X[start : start + n], y[start : start + n]

    def _pool_size(self) -> int:
        # Fixed for the standard splits so the RNG stream (and thus sklearn's hidden
        # generating function) is identical no matter which split is being built.
        return (max(split_offset(self.split), 2) + 1) * self.config.num_samples

    def _generate_classification_data(self) -> Tuple[np.ndarray, np.ndarray]:
        """Generate synthetic classification data (same class structure for every split)"""
        X, y = make_classification(
            n_samples=self._pool_size(),
        ...
        return self._split_slice(X, y)

    def _generate_regression_data(self) -> Tuple[np.ndarray, np.ndarray]:
        """Generate synthetic regression data (same linear function for every split, unit-scale targets)"""
        X, y = make_regression(
            n_samples=self._pool_size(),
            n_features=self.config.num_features,
            n_informative=self.config.num_informative,
        ...
        y = y.flatten()
        y = (y - y.mean()) / (y.std() + 1e-8)  # unit scale so MSE/MAE are interpretable
        rng = np.random.RandomState(self.config.random_state + 1000)
        y = y + rng.normal(0.0, self.config.noise_level, size=y.shape)
        return self._split_slice(X, y)
...
class ComplexTabularDataset(Dataset):
    ...
    def _generate_complex_data(self) -> pd.DataFrame:
        ...
        np.random.seed(self.config.random_state + split_offset(self.split))
...
class TimeVaryingTabularDataset(Dataset):
    ...
    def __init__(
        ...
        self.targets_tensor = torch.tensor(
            self.targets, dtype=torch.long if task == "classification" else torch.float32
        )
