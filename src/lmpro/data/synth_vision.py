...
def seed_split(split: str, base: int = 42) -> int:
    """Seed ``random``, ``numpy`` and ``torch`` for a split and return the seed used"""
    seed = base + split_offset(split)
    random.seed(seed)
    np.random.seed(seed)
...
class SyntheticImageDataset(Dataset):
    ...
    def __init__(self, config: VisionDatasetConfig, split: str = "train", transform=None):
        self.config = config
        self.split = split
    ...
    for i in range(num_samples):
        ...
        if isinstance(target, torch.Tensor):
            if target.dim() == 2:  # Segmentation mask
                ...
                axes[1, i].text(0.5, 0.5, f"Class: {target.item()}", ha="center", va="center", fontsize=12)
