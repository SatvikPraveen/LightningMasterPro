...
class TabularDataModule(LightningDataModule):
            ...
            for i in range(min(4, self.num_features)):
                ...
                if self.task == "classification":
                    scatter = axes[i].scatter(
                        range(len(feature_values)), feature_values, c=target_values, cmap="tab10", alpha=0.7
                    )
    ...
    def get_feature_importance_data(self) -> Dict[str, torch.Tensor]:
        ...
        return {"features": features_tensor, "targets": targets_tensor, "feature_names": self.get_feature_names()}
...
def get_regression_datamodule(
    num_features: int = 20, batch_size: int = 64, dataset_type: str = "simple", **kwargs
) -> TabularDataModule:
    """Get a tabular datamodule for regression"""
    config = TabularDatasetConfig(num_features=num_features, **kwargs)

    return TabularDataModule(task="regression", data_config=config, batch_size=batch_size, dataset_type=dataset_type)


def get_classification_datamodule(
    num_features: int = 20, num_classes: int = 3, batch_size: int = 64, dataset_type: str = "simple", **kwargs
) -> TabularDataModule:
    """Get a tabular datamodule for classification"""
    config = TabularDatasetConfig(num_features=num_features, num_classes=num_classes, **kwargs)
