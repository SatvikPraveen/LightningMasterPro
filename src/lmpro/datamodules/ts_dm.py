...
class TimeSeriesDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        # Shape info (refined from real samples in setup())
        ...
        for i in range(min(num_samples, len(sequences))):
            ...
            if sequence.dim() == 1:  # Univariate
                ...
                for feature_idx in range(min(3, sequence.shape[1])):  # Plot up to 3 features
                    axes[i].plot(sequence[:, feature_idx].numpy(), label=f"Feature {feature_idx}", alpha=0.7)
            ...
            if self.task == "forecasting":
                ...
                if target.dim() == 0:  # Single point forecast
                    ...
                    axes[i].scatter([forecast_start], [target.item()], color="r", s=50, label="Target")
                else:  # Multi-step forecast
                    forecast_indices = range(forecast_start, forecast_start + len(target))
                    axes[i].plot(forecast_indices, target.numpy(), "r--", linewidth=2, label="Target Forecast")
            ...
            if self.task != "forecasting":
                axes[i].set_title(
                    f"Sample {i} - Label: {target.item() if target.dim() == 0 else target.argmax().item()}"
                )
    ...
    def compute_sequence_statistics(self) -> Dict[str, Any]:
        ...
        if self.task == "forecasting":
            ...
            stats["class_distribution"] = {
                f"class_{cls.item()}": count.item() for cls, count in zip(unique_classes, counts)
            }

        return stats

    def create_forecast_visualization(
        self, predictions: torch.Tensor, targets: torch.Tensor, inputs: torch.Tensor, num_samples: int = 3
    ) -> None:
        ...
        for i in range(min(num_samples, len(predictions))):
            ...
            if input_seq.dim() == 1:  # Univariate
                axes[i].plot(range(input_length), input_seq.numpy(), "b-", label="Input", linewidth=2)
            else:  # Plot first feature for multivariate
                axes[i].plot(range(input_length), input_seq[:, 0].numpy(), "b-", label="Input (Feature 0)", linewidth=2)

            # Plot predictions and targets
            forecast_indices = range(input_length, input_length + len(pred_seq))

            if pred_seq.dim() == 0:  # Single point
                axes[i].scatter([input_length], [pred_seq.item()], color="red", s=50, label="Prediction")
                axes[i].scatter([input_length], [target_seq.item()], color="green", s=50, label="Target")
            else:  # Multi-step
                axes[i].plot(forecast_indices, pred_seq.numpy(), "r--", label="Prediction", linewidth=2)
                axes[i].plot(forecast_indices, target_seq.numpy(), "g--", label="Target", linewidth=2)
...
def get_forecasting_datamodule(
    ...
    **kwargs,
    ...
    config = TimeSeriesDatasetConfig(
        sequence_length=sequence_length, prediction_horizon=prediction_horizon, num_features=num_features, **kwargs
    )

    return TimeSeriesDataModule(
        task="forecasting", data_config=config, batch_size=batch_size, dataset_type=dataset_type
    )


def get_anomaly_detection_datamodule(
    sequence_length: int = 50, num_features: int = 1, batch_size: int = 32, **kwargs
) -> TimeSeriesDataModule:
    """Get a time series datamodule for anomaly detection"""
    config = TimeSeriesDatasetConfig(sequence_length=sequence_length, num_features=num_features, **kwargs)

    return TimeSeriesDataModule(task="anomaly", data_config=config, batch_size=batch_size, dataset_type="anomaly")


def get_timeseries_classification_datamodule(
    sequence_length: int = 50, num_features: int = 1, batch_size: int = 32, dataset_type: str = "univariate", **kwargs
) -> TimeSeriesDataModule:
    """Get a time series datamodule for classification"""
    config = TimeSeriesDatasetConfig(sequence_length=sequence_length, num_features=num_features, **kwargs)

    return TimeSeriesDataModule(
        task="classification", data_config=config, batch_size=batch_size, dataset_type=dataset_type
    )
